...
export default function TaskDiagramPage({ params }: PageProps) {
  ...
  // React Flow change handlers (REQUIRED for draggability)
  ...
  const task = tasks.find((t: any) => t.id === resolvedParams.id);
  ...
  const buildDiagram = useCallback(() => {
    ...
    //   assigned_staff_structure: task.assigned_staff?.map((a: any) => ({
    //     id: a.id,
    //     staff_id: a.staff_id,
    ...
    if (task.support_files && task.support_files.length > 0) {
      ...
      task.support_files.forEach((fileUrl: any, index: number) => {
    ...
    if (task.allocation_mode === 'team' && task.assigned_team_ids?.length > 0) {
      ...
      //   assigned_staff_count: task.assigned_staff?.length || 0,
      //   assigned_staff: task.assigned_staff?.map((a: any) => ({
      //     id: a.id,
      //     staff_id: a.staff_id,
      //     staff_name: a.staff?.name
      //   }))
      ...
      if (task.assigned_staff && task.assigned_staff.length > 0) {
        ...
        task.assigned_staff.forEach((assignment: any, index: number) => {
          ...
          if (assignment.staff) {
            ...
            const isLeader = task.assigned_teams?.some((team: any) => 
    ...
    } else if (task.allocation_mode === 'individual' && task.assigned_staff_ids?.length > 0 && !task.has_delegations) {
      ...
      task.assigned_staff_ids.forEach((staffId: string, index: number) => {
        ...
        const staffMember = task.assigned_staff?.find((a: any) => a.staff_id === staffId);
    ...
    // CLEAN Sequential delegation chain visualization
    if (task.has_delegations && task.delegations && task.delegations.length > 0) {
      // console.log('🔗 Creating clean sequential delegation flow', {
      //   original_assignee: task.original_assignee,
      //   current_assignee: task.delegation_chain[task.delegation_chain.length - 1]
      // });
      
      const DELEGATION_START_Y = 350; // Start position below task
      const DELEGATION_VERTICAL_SPACING = 250; // Vertical gap between cards
      
      // 1. Create Original Staff Node
      const originalStaffNode: Node = {
        id: 'original-staff',
        type: 'memberNode',
        position: { 
          x: TASK_X, // Same X as task (centered)
          y: DELEGATION_START_Y 
        },
        draggable: true,
        data: {
          label: task.delegations[0]?.from_staff?.name || 'Unknown',
          schema: [
            { title: "Role", value: "Original Assignee" },
            { title: "Status", value: "First assigned this task" },
          ],
          isLeader: false,
          isOriginal: true,
          isCurrent: false,
          profileImage: task.delegations[0]?.from_staff?.profile_image_url || undefined,
        },
      };
      
      newNodes.push(originalStaffNode);
      
      // 2. Create edge from Task to Original Staff
      newEdges.push({
        id: 'task-to-original',
        source: 'task',
        target: 'original-staff',
        type: 'smoothstep',  // ✅ Better visual for sequential flow
        animated: true,
      ...
      // 3. Create Current Staff Node
      const currentStaff = task.delegations[task.delegations.length - 1];
      const currentStaffImage = currentStaff?.to_staff?.profile_image_url;
      const currentStaffNode: Node = {
        id: 'current-staff',
        type: 'memberNode',
        position: { 
          x: TASK_X, // Same X as task (centered)
          y: DELEGATION_START_Y + DELEGATION_VERTICAL_SPACING // Below original
        },
        draggable: true,
        data: {
          label: currentStaff?.to_staff?.name || 'Unknown',
          schema: [
            { title: "Role", value: "Current Assignee" },
            { title: "Delegated By", value: currentStaff?.from_staff?.name || 'Unknown' },
            ...(currentStaff?.created_at ? [{ 
              title: "Date", 
              value: new Date(currentStaff.created_at).toLocaleDateString() 
            }] : []),
          ],
          isLeader: false,
          isOriginal: false,
          isCurrent: true,
          profileImage: currentStaffImage || undefined,
        },
      };
      
      newNodes.push(currentStaffNode);
      
      // 4. Create edge from Original to Current Staff
      newEdges.push({
        id: 'original-to-current',
        source: 'original-staff',
        target: 'current-staff',
        type: 'smoothstep',  // ✅ Better visual for sequential flow
        animated: true,
        style: { stroke: '#22c55e', strokeWidth: 3 },
        label: `Delegated${currentStaff?.notes ? `: ${currentStaff.notes}` : ''}`,
