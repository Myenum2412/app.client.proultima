...
export function RealtimeDataSync() {
  ...
  useEffect(() => {
    ...
    const maintenanceChannel = supabase
      .channel('admin-maintenance-sync')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'maintenance_requests' },
        (payload) => {
          // console.log('📡 Supabase: Maintenance request changed', payload.eventType);
          handleUpdate();
    ...
    const purchaseChannel = supabase
      .channel('admin-purchase-sync')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'purchase_requisitions' },
        (payload) => {
          // console.log('📡 Supabase: Purchase requisition changed', payload.eventType);
          handleUpdate();
