...
export function MaintenanceReportPDF({ requests }: MaintenanceReportPDFProps) {
  ...
  const getRunningStyle = (status: string) => {
    return status === 'running'
      ? [styles.runningBadge, styles.runningYes]
      : [styles.runningBadge, styles.runningNo];
  };
  ...
  const getRunningColor = (status: string) => {
    return status === 'running'
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>Maintenance Requests Report</Text>
          <Text style={styles.subtitle}>System Maintenance Records and Status</Text>
        ...
        <View style={styles.table}>
          <View style={styles.tableHeader}>
            ...
            <Text style={[styles.tableHeaderCell, { width: '20%' }]}>Brand Name</Text>
            <Text style={[styles.tableHeaderCell, { width: '15%' }]}>Workstation</Text>
            <Text style={[styles.tableHeaderCell, { width: '15%' }]}>Status</Text>
            <Text style={[styles.tableHeaderCell, { width: '12%' }]}>Running</Text>
          ...
          {requests.map((request, index) => (
            <View key={request.id} style={styles.tableRow}>
              ...
              <Text style={[styles.tableCell, { width: '20%' }]}>{request.serial_number || '-'}</Text>
              <Text style={[styles.tableCell, { width: '20%' }]}>{request.brand_name || '-'}</Text>
              <Text style={[styles.tableCell, { width: '15%' }]}>{request.workstation_number || '-'}</Text>
              <Text style={[styles.tableCell, { width: '15%', ...getStatusColor(request.status) }]}>
                {request.status}
              </Text>
              <Text style={[styles.tableCell, { width: '12%', ...getRunningColor(request.running_status) }]}>
                {request.running_status}
              </Text>
