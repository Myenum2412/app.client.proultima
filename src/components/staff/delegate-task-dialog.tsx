...
export function DelegateTaskDialog({ task, availableStaff }: DelegateTaskDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1b. Insert notification for the staff member who received the delegation
      const { data: fromStaffData } = await supabase
        .from('staff')
        .select('name')
        .eq('id', user?.staffId)
        .single();
      ...
      await supabase
        ...
        .insert({
          ...
          metadata: {
            ...
            delegated_by: fromStaffData?.name || user?.name,
            delegation_reason: notes
          }
        });

      // 2a. Delete old task_assignment for delegator (sync task_assignments table)
      ...
      if (deleteAssignmentError) {
        console.error('Failed to remove old assignment:', deleteAssignmentError);
        // Continue anyway - task might not have had an assignment entry
      }

      // 2b. Create new task_assignment for delegated staff (sync task_assignments table)
      ...
      if (createAssignmentError) {
        console.error('Failed to create new assignment:', createAssignmentError);
        throw createAssignmentError; // This is critical - must succeed
      }

      // 2c. Update task: remove current user, add new staff (update assigned_staff_ids array)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm">
          ...
          <span className='max-sm:hidden'>Delegate</span>
        </Button>
      </DialogTrigger>
