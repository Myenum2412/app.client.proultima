...
export function TaskAllocationDialog({ trigger, isOpen: controlledOpen, onOpenChange }: TaskAllocationDialogProps) {
  ...
  // Transform staff to match expected interface and filter out current user for staff
  const employees = staff
    .filter(s => {
      // If user is staff, filter out themselves from assignable staff
      if (user?.staffId && s.id === user.staffId) {
        return false;
      }
      return true;
    })
  ...
  // Check if user is staff (has staffId) vs admin
  const isStaffUser = !!user?.staffId;
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button className="w-1/2 sm:w-64 flex items-center justify-center gap-2">
            <IconCirclePlusFilled className="h-4 w-4" />
            <span>Tasks Allocate</span>
          </Button>
        )}
      </DialogTrigger>

      <DialogContent 
        className="w-[95vw] max-w-[800px] max-h-[90vh] overflow-y-auto p-4 sm:p-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
        style={{
          scrollbarWidth: 'none',
                      ...
                      return team ? (
                        ...
                        >
                          ...
                          <span className="text-xs opacity-70">
                            ({(teamMembers?.filter(tm => tm.team_id === teamId).length || 0) + 1} members)
                          </span>
                          <button
                            type="button"
                        ...
                        <SelectItem key={team.id} value={team.id}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              ({(teamMembers?.filter(tm => tm.team_id === team.id).length || 0) + 1} members)
                            </span>
                          </div>
                        </SelectItem>
                      ))}
                  ...
                  <SelectContent>
                    {priorities.map((priorityOption) => (
                      <SelectItem key={priorityOption.id} value={priorityOption.name}>
                        {priorityOption.name.charAt(0).toUpperCase() + priorityOption.name.slice(1)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
