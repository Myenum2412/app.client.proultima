...
export function useTeams() {
  ...
  const { data: teams = [], isLoading, error, refetch } = useQuery<Team[]>({
    ...
    staleTime: 3 * 60 * 1000, // 3 minutes - team structure changes less frequently
  ...
  const invalidateTeams = () => {
    ...
    broadcastDataUpdate('teams-updated');
  };

  // Listen for cross-tab sync via Broadcast Channel
  useEffect(() => {
    const unsubscribe = subscribeToBroadcast((message) => {
      if (message.type === 'teams-updated') {
        // console.log('🔄 Cross-tab sync: teams updated');
        queryClient.invalidateQueries({ queryKey: ['teams'] });
        queryClient.invalidateQueries({ queryKey: ['team-members'] });
      }
    });
    ...
    const teamsChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          invalidateTeams(); // Use instant invalidation
        }
      )
      .subscribe((status) => {
